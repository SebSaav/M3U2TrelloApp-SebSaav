...
const createTask = (task) => {
    ...
    taskResponsible.innerHTML = `<span class="card_task__responsible--tag-creator"><strong>Responsable:</strong></span> ${task.person}`;
  
    let taskDetails = document.createElement("p");
    taskDetails.classList.add("card-task__details");
    taskDetails.innerHTML = `<span class="card-task__details--task-details"><strong>Descripción:</strong></span> ${task.details} `;
  
    let taskDate = document.createElement("p");
    taskDate.classList.add("card-task__date");
    taskDate.innerHTML = `<span class="card-task__date--tag-date"><strong>Plazo:</strong></span> ${dateFormat(
      task.deadline
    )}`;
  
    let taskCreate = document.createElement("p");
    taskCreate.classList.add("card-task__date");
    taskCreate.innerHTML = `<span class="card-task__date--tag-date"><strong>Creación:</strong></span> ${dateFormat(
      task.created
    )}`;
